...
mod iso8583_parser {
    ...
    impl DataElement {
        fn new(position: usize, data_type: FieldType, header_size: usize, max_len: usize) -> DataElement {
            DataElement {
                position: position,
                data_type: data_type,
    ...
    impl Parser {
        ...
        pub fn new(iso_elements_spec: &BTreeMap<usize, String>) -> Parser {
            ...
            for (pos, value) in iso_elements_spec {
                ...
                elements_spec.insert(*pos, DataElement::new(*pos, field_type, header_len, max_len));
        ...
        pub fn parse_isomsg(&self, bytes: &[u8]) -> Txn {
            ...
            for (i, c) in bitmap_binstr.chars().enumerate() {
                if c == '1' {
                    ...
                    let (field_val, next_data) = parse_field(&iso_data, *header_len, *max_len, field_type);
    ...
    pub fn parse_field<'a>(bytes: &'a [u8], header_len: usize, max_length: usize, field_type: FieldType) -> (String, &'a [u8]) {
        
        let total_char_per_byte = match field_type {
                FieldType::A | FieldType::S => 1,
                _ => 2,
            };
        
        let (head_len, data_len) = 
            if header_len > 0 {
    ...
    pub fn split_bitmap_data(iso_msg: &[u8]) -> (&[u8], &[u8]) {
    ...
    pub fn split_main_components(bytes: &[u8]) -> (&[u8], &[u8], &[u8]) {
        let mti = &bytes[0..2];
        let (bitmap, data) = split_bitmap_data(&bytes[2..]);
        (mti, bitmap, data)
    }
...
fn main() {
    ...
    // add information if the format is ascii or bcd
    let field_format = BTreeMap::from([
        (2, (2, 0)),
        (7, (0, 10)),
        (22, (0, 3)),
        (63, (3, 0)),
